...
///Error! 因为age有外部参数，名字_ ,所以内部参数age不能再使用
///name不能省略，因为name是方法的第一个参数，不能作为外部参数
p.setName(name: "hjq", 88)  //正确姿势
p.show()

p.setName(name: "hjq2", 288)  //正确姿势
p.show()
...
setName(name: "han", age: 30)  //默认没有外部参数
...
struct Person4 {
    ...
    func show()
    {
        print("非静态方法，name = \(name) card = \(Person4.card)")
    }
    static func staticShow()
    {
        //类方法中没有self
        print("静态方法对应OC中的+号方法, 和OC一样在类方法中不能访问非静态属性")
...
var p5 = Person4(name:"gongchenjie" )
p5.show()
Person4.card = "88888"
//var Person4.card2 = "22222" Error
Person4.staticShow()
print(Person4.card )

//var p5.card2 = 333 Error
